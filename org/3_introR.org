...
5. Create an R test file =test.R=, list and view it:
   #+begin_src bash :results output :exports both
...
- Open the R console, either in a dumb or in a smart terminal (=M-x
  shell)=, or in a dedicated R buffer (=*R*=).
...
- Enter ~demo(graphics)~ for some graphics examples.
...
- Use ~setwd()~ to change directory to your user home directory (=$HOME=):

  1) Using an absolute path address: from the root directory (=/=)
     #+begin_src R
       setwd("../../../")
       getwd()
     #+end_src
  ...
  2) Using a relative path address: from the current location (=.=)
     #+begin_src R
       setwd("/home/aletheia")
       getwd()
     #+end_src
...
- The function =options= controls all glocal options for R:
  #+begin_src R :session *R* :results output :exports both
    help(options)
  #+end_src
...
- You can extract display options with ~$~, e.g. for the R console
  prompt:
  #+begin_src R :session *R* :results output :exports both
    options()$prompt
  #+end_src
...
- Another important option setting is for the repository that R uses
...
- The change affects only your current R session. Change to the ~*R*~
...
- On the R console, change the prompt back to what it was.
  #+begin_example
  R> 
  R> options(prompt="> ")
  > 
