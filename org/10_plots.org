...
3) Save the file [[https://tinyurl.com/people-csv][tinyurl.com/people-csv]] as a data frame using
   ~read.csv~. Look at the file to check if ~header~ should be ~TRUE~ or
   ~FALSE~, and set ~stringsAsFactors=TRUE~.
p
   #+begin_src R :results silent
     df <- read.csv(file="https://tinyurl.com/people-csv",
                    header=TRUE,
...
4) Save the file again as a data frame ~dnf~ but this time do *not* set
   ...
   #+begin_src R
     dnf <- read.csv(file="https://tinyurl.com/people-csv",
                     header=TRUE)
     str(dnf)
   #+end_src
   ...
   :  $ Weight: int  68 44 66 46 40 76 93 69 69 64 ...
   :  $ Height: int  197 200 177 169 193 191 185 200 181 193 ...
   :  $ Sex   : chr  "male" "male" "male" "male" ...
   :  $ Name  : chr  "Evan" "Ian" "Ivy" "Carla" ...
   :  $ Age   : int  23 55 26 25 65 78 48 49 75 69 ...
...
6) Check the structure of the data frame ~df~.
   ...
   :  $ Name  : Factor w/ 20 levels "Alex","Alice",..: 10 17 18 6 7 4 20 7 14 7 ...
   :  $ Age   : int  23 55 26 25 65 78 48 49 75 69 ...

7) Print the top of the data frame.
   #+begin_src R
     df |> head()
   #+end_src

   #+RESULTS:
   :   Weight Height    Sex  Name Age
   : 1     68    197   male  Evan  23
   : 2     44    200   male   Ian  55
   : 3     66    177   male   Ivy  26
   : 4     46    169   male Carla  25
   : 5     40    193 female Derek  65
   : 6     76    191   male Brian  78
...
9) What information does ~sex.freq~ contain? Write your answer below (as
   a full sentence).
   #+begin_quote
   The data frame contains observations on 506 female and 494 male
   participants.
   #+end_quote
...
11) Annotate the barplot by adding the title "Barplot of the data set
    ...
    #+begin_src R :results graphics file :file ../img/9_sex_ann.png
      barplot(
        ...
        xlab = "Participants",
        ylab = "Frequency",
        main = "Barplot of the data set people.csv",
        las = 1)
    #+end_src
...
- Histogram with customization
  #+begin_src R :results graphics file :file ./img/9_hphist.png
    h <- hist(mtcars$hp, #1
              main="Histogram of horsepower in mtcars", #2
              xlab="Horsepower [hp]",     #3
              ylab="Number of cars")   #4
    h
  #+end_src

  #+RESULTS:
  [[file:./img/9_hphist.png]]

- Print the binwidth of the last plot
  #+begin_src R
    h$breaks[2]-h$breaks[1]  #5
  #+end_src
...
- Logarithmic transformation:
  #+begin_src R :results graphics file :file ./img/9_hphist1.png
    hp <- mtcars$hp
    hist(log10(hp),  #6
         main="Histogram of horsepower in mtcars",
         xlab="Log(Horsepower) [hp]",
         ylab="Number of cars")
  #+end_src

  #+RESULTS:
  [[file:./img/9_hphist1.png]]

- Horsepower greater or equal than 220 hp:
  #+begin_src R :results graphics file :file ./img/9_hphist2.png
    hp <- mtcars$hp
    sub <- 220  # store cut off value
    idx <- which(hp >= sub)
    hpsub <- hp[idx]  # hp subset
                                            #   hist(mtcars$hp[which(mtcars$hp>=220)]) full expression
    hist(hpsub,  #7
         main="Histogram of horsepower in mtcars",
         xlab=paste("Horsepower >", sub,"[hp]"),   #8
         ylab="Number of cars")
  #+end_src

  #+RESULTS:
  [[file:./img/9_hphist2.png]]

- Put the log10 plot and the original plot in one plot array (on top
  of one another) - without any customization.
  #+begin_src R :results graphics file :file ./img/9_hphist3.png
    par(mfrow=c(2,1))
    hist(hp)
    hist(log10(hp))
  #+end_src
  ...
  #+RESULTS:
  [[file:./img/9_hphist3.png]]

- Put the log10 plot and the original plot in one plot array (side by
  side) - without any customization.
  #+begin_src R :results graphics file :file ./img/9_hphist4.png
    par(mfrow=c(1,2))
    hist(hp)
    hist(log10(hp))
  #+end_src
