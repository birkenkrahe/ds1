...
* Week 10 - Sequencing, repeating and subsetting vectors
#+attr_html: :width 600px:
#+caption: Claude Monet, Cap d'Antibes, Mistral (1888)
[[../img/Monet_Cap_dAntibes_Mistral_1888.jpg]]

- [ ] Review of the (failed) vector plotting practice
- [ ] Review of the DataCamp lesson on "Data Frames"
- [ ] Review of the 2nd sprint results "Literature Review"
- [ ] Vectors in R: sequences, repetitions, sorting, measuring
- [ ] Vectors in R: short lab practice (Canvas upload)
...
- Remember to take or keep up your DataCamp practice!
...
2. How do you select specific rows and columns from a =data.frame=? For
...
3. How do you sort the values of a =data.frame= by a specific column in
   descending order? For example the =mpg= column of =mtcars=?
   #+begin_src R
     sort(mtcars$mpg,decreasing=TRUE)
   #+end_src

4. How do you filter those elements of =mtcars= with more than 25
   miles-per-gallon?
   #+begin_src R
...
5. Which cars satisfy this condition?
   #+begin_src R
     which(mtcars$mpg > 25) -> rows25  # row index vector
     rows25  
     rownames(mtcars[rows25,]) # select row names
   #+end_src
...
6. Which function is used to create a =data.frame=?
   #+begin_src R
     data.frame(
       nums=c(1,2,3),
       name=c("Jim","Joe","Jane"),
