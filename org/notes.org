...
*Poll:* Did anyone listen to the last two podcasts or videos?

*Popquiz*: Vectors & Matrices & =NA= extension
...
14. Compute the average temperature of =temp_m2= using =mean=:
    #+begin_src R
      ...
      mean(temp_m2[1,])  # does not generalize (need to know where NA are)
    #+end_src
...
15. Compute the average temperature with =mean= only for the days where
    the temperature was greater or equal 80 degrees Fahrenheit:
    #+begin_src R
      idx <- na.omit(temp_m2)>=80
      idx
      mean(temp_m2[1,-which(idx)], na.rm=TRUE)
    #+end_src
