...
- The final presentation should contain a "discussion" (see image):
  ...
  2) Findings (How do the findings relate to the literature?)
  3) Outlook (What else could or should one do now?)
  4) Limitations (What limits the importance of this research?)
...
3. Why are the rows of the =data.frame= numbered?
   #+begin_quote
...
4. If two =numeric= vectors =foo= and =bar= have =length= 10 and 6, how can I
   ...
   #+begin_src R
     ...
     c(bar,
       rep(x=NA,
           length.out=length(foo2)-length(bar2))) -> bar4
     data.frame(foo2,bar4)
   #+end_src
...
- [ ] Statistical functions on subsets
- [ ] Logical functions in vectors
- [ ] Selection with logical index vectors
- [ ] Negative indices
