...
* TODO Review questions (DataLab & Linux)
...
7. Which software do you need to run R inside Emacs (as in =testR.org=)?
   ...
   3. ESS package installed in Emacs (with =list-packages=)
   4. =.emacs= file downloaded (with =wget=)
   #+end_quote
...
* TODO Review questions (DataCamp - Data Collection and Storage)
...
4) What are the main types of data and how are they different?
   #+begin_quote
...
5) What do you need to consider when storing data?
   #+begin_quote
   - Location (e.g. parallel storage solutions or the cloud)
   - Data type (e.g. unstructured or tabular)
...
* TODO Introduction to data science (lecture)
